...
class EnsemblePursuitPyTorch():
    ...
    def threshold(self,C,X,seed):
        #norm_C=torch.norm(C,p=2,dim=0)
        #norm_max_C=torch.argmax(norm_C)
        C_sub=C[:,seed]
        #norm_X=torch.norm(X,p=2,dim=1)
        #norm_max_X=torch.argmax(norm_X)
        #X_sub=X[norm_max_X,:]
        X_sub=X[seed,:]
        coef=torch.clamp(C_sub,min=0,max=None)**2/(self.sz[1]*((X_sub**2).sum()))
        values,elements=coef.sort()
        threshold=values[-25].item()
        return threshold

    def fit_one_ensemble(self,X):
        C=X@X.t()
        #threshold=self.threshold(C,X)
        #print(threshold)
        #self.lambd=threshold
        ...
        while n<min_assembly_size:
            ...
            threshold=self.threshold(C,X,seed)
            print(threshold)
            self.lambd=threshold
            n=1
