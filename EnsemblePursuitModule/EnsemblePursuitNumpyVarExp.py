...
class EnsemblePursuitNumpyVarExp():
    ...
    def calculate_cost_delta(self,C_summed,current_v):
        cost_delta=np.clip(C_summed,a_min=0,a_max=None)**2/((current_v**2).sum())
        return cost_delta
    ...
    def fit_one_ensemble(self,X):
        ...
        while n<min_assembly_size:
            ...
            while max_cost_delta>0:
                ...
                n+=1
                current_v_=(1./n)*current_v_unnorm
                var_exp_threshold=self.calculate_variance_explained(current_v,current_v_)
                if var_exp_threshold==True:
