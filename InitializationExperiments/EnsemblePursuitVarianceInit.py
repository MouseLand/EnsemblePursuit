...
class EnsemblePursuitVarianceInit():
    ...
    def fit_ICA(self,X):
        ...
        seed_initializations = transformer.fit_transform(np.array(X.t().cpu()))
        print(seed_initializations.shape)
        return seed_initializations
    
    def calculate_first_neuron(self,X,seed_timecourse):
        delta=torch.clamp(X@seed_timecourse,min=0,max=None)**2/(seed_timecourse**2).sum()
        max_delta_neuron=delta.argmax()
        return max_delta_neuron
    ...
    def fit_one_ensemble(self,X):
        ...
        #Seed current_v
        max_delta_neuron=torch.argmax(torch.var(X,dim=1))
        current_v=X[max_delta_neuron,:].flatten()
    ...
    def fit_transform(self,X):
        ...
        self.U=torch.ones((X.size(0),1))
        self.V=torch.ones((1,X.size(1)))
