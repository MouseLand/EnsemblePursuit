...
class EnsemblePursuitThresholdInit():
    ...
    def fit_one_ensemble(self,X,C,top_neurons):
    ...
    def fit_transform(self,X):
        ...
        for iteration in range(0,self.n_ensembles):
            current_u, current_v=self.fit_one_ensemble(X,C,top_neurons)
            ...
            print('cost',cost)
            C=X@X.t()
            top_neurons=torch.cat((top_neurons[-1:], top_neurons[:-1]))
