...
class EnsemblePursuitNumpyFast():
    ...
    def fit_one_ensemble(self,C,X,neurons_for_seed):
        C_lim=C[neurons_for_seed[:,None],neurons_for_seed[None,:]]
        ...
        while n<min_assembly_size:
            if self.n_neurons_for_sampling>1:
                 seed=self.repeated_seed(C_lim,neurons_for_seed)
            else:
                 seed=self.sorting_for_seed(C_lim,neurons_for_seed)
            n=1
            current_v=X[seed,:]
    ...
    def repeated_seed(self,C_lim,neurons_for_seed):
        sorted_similarities=np.sort(C_lim,axis=1)[:,:-1]
        ...
        top_neurons=top_neurons[neurons_for_seed.shape[0]-(self.n_neurons_for_sampling):]
        seed=top_neurons[sample_top_neuron][0]
        return neurons_for_seed[seed]

    def sorting_for_seed(self,C_lim,neurons_for_seed):
        '''
    ...
    def neurons_for_seed(self,C):
        nr_neurons_to_av=self.options_dict['seed_neuron_av_nr']
        sorted_similarities,_=np.sort(C,axis=1)[:,:-1][:,self.sz[0]-nr_neurons_to_av-1:]
        sorted_similarities=sorted_similarities 
        average_similarities=np.mean(sorted_similarities,axis=1)
        neurons_for_seed=np.argsort(average_similarities)
        return neurons_for_seed[-500:]
    ...
    def fit_transform(self,X):
        ...
        start=time.time()
        C=X@X.T
        neurons_for_seed=self.neurons_for_seed(C)
        for iteration in range(0,self.n_ensembles):
            current_u, current_v=self.fit_one_ensemble(C,X,neurons_for_seed)
